...
We also retrieved certain quantifiable indicators on the aforementioned recorded variables. Ranges of motion and onset times were examined for translations, and peak values and times for angles and speeds. We computed mean differences and standard deviation (std) between marker-based results and all other protocol results. The impact of the camera type, and of the 2D pose estimation model, were similarly explored. 

Normality of the distribution of differences was verified with a Shapiro-Wilk test 

, we used paired t-tests to  Significance of differences among

As differences were not normally distributed, paired t-test could not be rigorously performed; and as they were more frequently null or tied than not, Wilcoxon signed-rank tests could not be used either. As a consequence, we could not reject the null hypothesis that results are identical in average. 
However, we could still compare the magnitude of differences between protocols, and between camera types and 2D pose estimation models.



% the differences between the reference and the markerless methods. We also used the Wilcoxon signed-rank test to compare the differences between the reference and the markerless methods when normality was not verified. We used a significance level of 0.05.

% with the reference marker-based method was verified, we used paired t-tests to assess whether these differences were statistically significant. Lastly, we performed a Bland-Altman analysis, i.e., we reported mean differences and 95\% Limits of Agreement (95\% LoA) \cite{Bland1986}. 
...
\begin{figure}[!ht]
	...
	\caption{A representative trial by one of the athletes. Comparison of selected variables for a sequence of jab, high hook, and low hook in boxing. Waveforms look very similar across all protocols, even though the use of the standard OpenPose body\_25b model instead of the experimental body\_25b one seems to cause more discrepancy when compared to the reference marker-based analysis.}
	\label{fig_graphkpi}
\end{figure}
...
\begin{table}[!ht]
      ...
      \begin{tabular}{lcccc}
          ...
          Rear shoulder rot.\(^2\) (°) & -4.1 ($\pm$2.2) * & -2.7 ($\pm$2.9) * & -3.4 ($\pm$2.7) * & -0.9 ($\pm$6.2) \\
          ...
          Rear shoulder rot.\(^2\) (ms) & -58.3 ($\pm$52.1) * & -40.3 ($\pm$45.0) * & -56.4 ($\pm$59.6) & -15.3 ($\pm$55.5) \\
...
Almost always significantly different from  marker-based. Peak translations: up to about 10 cm differences. Peak velocities: up to about 0.4 m/s difference (), always underestimated. 
...
\begin{table}[!ht]
      ...
      \begin{tabular}{lcccc}
            ...
            \(\mathbf{Mean_{err}}\) \textbf{($\pm$std)} & \shortstack{\textbf{ROM\(^1\) \emph{or} Peak value}\\(mm, °, or m/s)} & \shortstack{\textbf{Onset\(^1\) \emph{or} Peak time}\\(ms)} & \shortstack{\textbf{ROM\(^1\) \emph{or} Peak value} \\(mm, °, or m/s)} & \shortstack{\textbf{Onset\(^1\) \emph{or} Peak time}\\(ms)} \\
            \cmidrule(l{2pt}r{2pt}){2-2} \cmidrule(l{2pt}r{2pt}){3-3}
            \cmidrule(l{2pt}r{2pt}){4-4} \cmidrule(l{2pt}r{2pt}){5-5}
            \textbf{Jab}\\
            Lead foot translation\(^1\) (mm) & 3.3 ($\pm$8.9) * & -23.1 ($\pm$18.9) * & 12.2 ($\pm$10.9) * & -10.2 ($\pm$16.1) *\\
            Pelvis translation\(^1\) (mm) & 13.3 ($\pm$10.4) * & -18.1 ($\pm$18.4)  & 4.8 ($\pm$8.2) * &-3.2 ($\pm$15.8) *\\
            Lead elbow extension (°) & -6.5 ($\pm$10.8) * & -7.4 ($\pm$16.6) * & -7.8 ($\pm$9.5) * & -8.3 ($\pm$17.1) *\\
            Lead fist velocity (m/s) & 0.1 ($\pm$0.2) * & -7.4 ($\pm$15.7) * & -0.1 ($\pm$0.2) * & -4.6 ($\pm$11.0) *\\
            \\
            \textbf{Hook}\\
            Rear knee flexion\(^2\) (ms) & 0.5 ($\pm$3.2) * & -2.8 ($\pm$29.8) * & -4.1 ($\pm$2.6) * & -31.9 ($\pm$58.8)\\
            Pelvis rotation (ms) &  -4.4 ($\pm$6.9) * & 5.5 ($\pm$59.8) & -3.5 ($\pm$5.2) * & 8.4 ($\pm$50.7)\\
            Rear shoulder rot.\(^2\) (ms) & 2.0 ($\pm$5.2) & 29.9 ($\pm$80.6) & 1.2 ($\pm$4.6) & 13.2 ($\pm$68.9) \\
            Rear fist velocity (ms)& -0.1 ($\pm$0.7) & -10.2 ($\pm$30.1) & -0.1 ($\pm$0.5) & -5.6 ($\pm$21.8)\\
            \bottomrule
      \end{tabular}}
...
Curves remarkably similar, but peak values, peak times, ranges of motion, and onset times more different (magnitude?)
Effect of camera grade and of 2D pose estimation protocol similar magnitude
...
GoPro 120 fps mode actually samples at 119.88 fps, i.e., 59.94 fps after resampling. This is a multiple of 29.97 fps, a holdout from the introduction of color on television in North America in the 1950s, and is also true for phone cameras and most recording devices. This leads to a 3.6 frames delay per minute (0.06 s) when compared to the true 60 fps of Qualisys cameras. This led to a temporal drift, and artificially hampered our results on peak time comparisons, which could be close to perfect. However, this is not a problem in practice, as long as all cameras involved in the capture shoot at the same framerate.
