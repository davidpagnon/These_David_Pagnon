...
% Comparer avec autres erreurs d'autres méthodes estimant lengths, peak velocities, peak angles, peak times
% Here, very specific moments rather than spatio-temporal parameters averaged over a full gait cycle (cf Kanko). -> much more sensitive to noise and to smaller errors. Durations vs. instants, average speeds vs peak instantaneous speeds. Ranges of motion // stride width and length (slightly larger: order of a cm vs order mm)
...
Contexte compliqué,
variables variées different body parts (upper and lower body), and different types of continuous variables (translations, rotations, joint angles, velocities), and different sorts of kinematic KPIs (ROMs, onset times, peak values and times).
