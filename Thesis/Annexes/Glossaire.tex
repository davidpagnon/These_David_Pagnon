...
%  In this context, there are usually not exactly as many known marker coordinates as unknown joint angles.
...
- If M = N, there are as many observations as degrees of freedom, e.g., as many markers coordinates as unknown joint angles. Then, an exact solution can be calculated. This is for example the case for single-body direct kinematics.
...
1. Calcul analytique direct (closed form) si connues = linear combinaison d'inconnues: f(x)-q peut s'ecrire AX - B. 
...
- damped http://graphics.cs.cmu.edu/nsp/course/15-464/Spring11/handouts/iksurvey.pdf
...
% generalized coordinates: not exclusively cartesian coordinates (X,Y,Z). Can also be angles. Generalized forces can also be moments
