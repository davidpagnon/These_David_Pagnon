...
Although there is no Graphical User Interface (GUI) yet, Pose2Sim requires only moderate Python skills, and each task is as fully and easily configurable as possible. By editing the \mintinline{shell-session}{User/Config.toml} file, and the user can specify the project path and folder names, the video frame rate, the range of analyzed frames; as well as the 2D pose estimation model used, and calibration, tracking, and triangulation parameters. 
