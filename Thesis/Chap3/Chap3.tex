...
We recommend using the OpenPose body\_25B model, as it provides foot keypoints, is as fast as the standard body\_25 one, and as we have extensively tested it \cite{Pagnon2022a}. Note that only 21 of the 25 detected keypoints are tracked, since eye and ear keypoints would be redundant in the determination of the head orientation. 
...
\begin{itemize}[itemsep=0em, topsep=0em, leftmargin=*]
      ...
      \item The minimum number of "good" cameras, \(cams_{min}\) (i.e., cameras remaining after the last two steps) required for triangulating a keypoint. If there are not enough cameras left, the 3D keypoint is dropped for this frame.
\end{itemize}
...
\begin{algorithm}[!ht]
      ...
      \begin{algorithmic}[0]
            \STATE Let $error_{max}$ be the maximum allowed reprojection error, $cams_{min}$ the minimum number of cameras demanded for triangulation, and $conf_{min}$ the minimum confidence accepted for a keypoint to be used.
            \WHILE {$error > error_{max}$ AND $cams > cams_{min}$}
                \FOR {all combinations $\binom{ncams}{cams}$}
                    \IF {$conf>conf_{min}$}
                        \STATE Add keypoint to equation system
                    \ENDIF
...
\end{itemize}
All weights of joint coordinates are set to 1, apart from those of the nose and the head, which are set to 0.2, and from those of the other head markers, which are set to 0. Since we process scaling during a standing pose, we add as a condition that ankle flexion should be fixed at a neutral 0° angle. 
