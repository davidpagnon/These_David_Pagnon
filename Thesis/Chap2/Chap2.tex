...
This system is now overdetermined. As it can be written in the form \(\textbf{A} \overrightarrow{X} = \overrightarrow{0}\), a linear-eigen pseudo-solution can be found \cite{Hartley2003} by using Singular Value Decomposition (SVD) \cite{Golub1971} (See Algorithm~\ref{alg:svd}). In order to fully determine $\textbf{P}$, it first needs to be normalized, then the least-square solution needs to be found, and then it can be denormalized. 

\newpage
\begin{algorithm}[!ht]
  \caption{Linear-eigen pseudo-solution of \(\textbf{A} \overrightarrow{X} = \overrightarrow{0}\)}\label{alg:svd}
  \begin{algorithmic}[1]
...
Rough 3D joint coordinates alone are not enough to provide an insightful understanding of human motion. Unless some assumptions are made, one single keypoint per joint can at best describe planar flexion/extension angles. This conceals any abduction/adduction or internal/external rotation angles. For this reason, it is important to find indirect ways to obtain 3D joint angles, from seemingly incomplete information.
...
The standard approach to obtain 3D joint angles is to use direct kinematics. 

% This consists in using the 3D coordinates of the joint and of its neighbors, and then applying a mathematical formula to calculate the joint angles. This is the approach used in \autoref{ch:6}, where we used the 3D coordinates of the shoulder, elbow and wrist to calculate the elbow flexion/extension angle. This approach is very efficient, but it requires a priori knowledge of the joint kinematics, which is not always available. Moreover, it is not always possible to obtain the 3D coordinates of all the joints, especially in the case of a crowded scene. In this case, it is possible to use indirect kinematics, which does not require the 3D coordinates of all the joints.

direct/forward:Analytical/6DoF free body/segment optimization
-> trust markers
...
assumes that markers are positionned very accurately, by one single trained operator, no STA.
renders dislocations (good if there are some, bad if none)
...
inverse:numerical=iterative/model based/global optimization
-> trust model
...
full-body: 14 segments, at least 3 markers per segment for 3D carac=42 mk. Openpose: only 21: need for good model
