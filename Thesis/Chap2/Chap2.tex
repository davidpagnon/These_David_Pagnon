...
In the field of biomechanics, inverse kinematics is usually dealt with Quasi-Newton methods. These methods don't linearize the objective function, and solve it as a more general non-linear minimization problem, which is not restricted to sum-of-squares objective functions. Let us call $h(\overrightarrow{q})$ be the objective function~\ref{eq_obj_ik} to minimize. A second order Taylor expansion of $h$ around $\overrightarrow{q}$ is:
\begin{equation}
...
% paragraphe intro cf feuille et dessous

% paragraphe theorie cf feuille et wiki en + BIORBD IK_KS L118  -> pas compris update

Another lesser-known class 
ne tire pas ses origines de la robotique, de l'animation, ni de la biomécanique, mais automation: Precisely to mitigate noisy measurements, when prior knowledge on expected model and process behavior are known
citer origine [Kalman,Swerling]
// Hidden markov, but continuous instead of discrete space 

Avoir idée de comment c'est smooth (et idée d'initialisation angles aussi, ou pas?): previous knowledge, not only of position, but also vel and acc.
Estimates simultaneously generalized coordinates, velocities, and accelerations -> less errors induces by differenciation 
Information from previous frame (and following with KS)


KF:
EKF:non linear systems (+accounts for kinematic constraints? )
LME: accounts for STA
KS: all times, back and forth. No account for STA. EKS extended
