...
The perceptron, invented in 1956 \cite{Rosenblatt1958}, represents the first practical application of an artificial neuron. It acts as a binary classifier, which automatically adjusts its weights by learning from example data (see Algorithm~\ref{alg:perceptron}). Adaline is a slightly improved 



It could be used, for example, to predict whether an athlete is going to be "good" or not, given his force-velocity results on an ergometer test (see step-by-step \hyperlink{example1}{Example 1} and Figure~\ref{fig_perceptron}). 


Logistic regression: Can be used if more than two classes need to be identified. Then the output is a probability distribution over the classes.

Multi-class: softmax, activation function for class k = exp()


These three algorithms are single layer Artificial Neural Network (ANN,) and are only valid for classifying linearly separable problems, with only two classes.

XOR problem, winter: multi-layer. We will not elaborate on them, since it is not the topic of this thesis.
...
Of course, the above example is oversimplified. Being good or not as a sport is a complex and multifactorial outcome, and two variables can't sum it up. It can also take a lot of iterations over good quality training data for the algorithm to converge, which it always does if and only if the data are linearly separable (see Figure~\ref{fig_linearly_sep}). For faster convergence, it is possible to play on the initial weights (although they are usually randomly chosen), on the learning rate (close enough to zero that it will not overstep the solution), on the activation function (), or on the error function.
...
More than two classes with logistic regression

% learning rate, the number of iterations, the number of layers, the number of neurons per layer, the activation function, the regularization, etc. to get a better result. But the main idea is there. The algorithm is able to find a decision boundary that separates the two classes. The decision boundary is a hyperplane in the 3D space, and it is represented by the equation:



The data need to be linearly separable

It always converges %\cite{}
...
\begin{algorithm}
      \caption{Adaline (and perceptron)}\label{alg:perceptron}
      \begin{algorithmic}[1]
        ...
        \STATEx \textit{In perceptron, $\theta=0$}.
        \begin{equation}
            \sigma^{0,pred} = 
            \begin{cases}
...
\begin{algorithm}
      \begin{algorithmic}[1]
        ...
        \end{equation}
        \STATEx with $\eta$ the learning rate $\in$ [0,1], and $\epsilon$ the error function such as \(\epsilon^0 = \sigma^{0,actual} - \sigma^{0,pred}\). 
        \STATEx \textit{In perceptron, \(\epsilon^0 = - \sigma^{0,pred}\)}.
...
\begin{tcolorbox}[colback=white,colframe=black, colbacktitle=white, coltitle=black, breakable, title=\textbf{Example 1} Athlete classification with Adaline, label=example1]
      \phantomsection\hypertarget{example1}
      N.B. The code for running this example is available on the thesis repository \url{https://github.com/davidpagnon/These_David_Pagnon/blob/main/Thesis/Chap2/adaline_perceptron.py}.
      \tcblower
