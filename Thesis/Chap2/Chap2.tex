...
Part 4.1, 4.3: https://sci-hub.se/https://doi.org/10.1115/1.4038741
https://sci-hub.se/10.1177/027836498800700501
https://sci-hub.se/https://pubmed.ncbi.nlm.nih.gov/24347565/
https://www.sciencedirect.com/science/article/abs/pii/S0021929016304134
https://journals.lww.com/acsm-msse/Fulltext/2014/07000/Impact_of_Knee_Modeling_Approach_on_Indicators_and.1.aspx


Free-body 6DoF: trust markers
Multibody IK: trust markers


Résultats similaires si STA limités et modèle approprié \cite{Kainz2016}

Aristidou2017: 
- 6DoF: if low DoF, not lots of STA, if looking at pathological individuals (missing  clinically relevant movements), needs to be fast
- IK: more general, if less markers. 
...
INVERSE KIN
-----------
Reduces sensitivity to soft tissue artifact, reduces number of degrees of freedom to solve, and thus of markers needed (1 per joint if fully constrained \cite{Slater2018}, vs. 3 min for 6DoF, no need for interpolation or redo capture if marker falls) -> Fohanno2014?, more potential information on muscle activation for example \cite{Robinson2013,Kainz2016}
Slower, assumes that all human beings have similar joint mechanics, based on \textit{a priori} constraints inferred from observations of a population, not guaranteed to work on pathological population nor on unusual body structures (joint laxity cannot be qualified for example)

Most exact: Quasi-Gaussian, L-BFGS-B (slowest) although last result used as a seed, close to optimum
Jacobian peuvent osciller, discontinuités, jerky near singular postures sauf pour damped least square, qui est plus lent (Aristidou)
Kalman filter (real time sauf KS, process model and initialization not trivial, can diverge. Estime simultanément positions, vitesse, accélérations. Constraints difficult to implement
Heuristic more computationally effcient but not accurate enough, Learning based not ready

SCALING
Although we just glanced over this aspect, scaling the model is indispensable for IK to reach des résultats corrects. Not trivial: besoin de trouver les joint centers, plusieurs manières
Model-based=kinematic chain (good enough? \cite{Hicks2015}. Neuromuscular, inertial properties, aspect not addressed here, as we are mostly interested in the kinematics at that stage).
1. Bons Dof (pin, ball, universeal, constrained, ou combinaison), pas trop mais assez, comprehensive vs reliable + only applicable with population close to the norm: limb missing, pathological joint, not okay
2. Bonnes dimensions, usually from bone joint centers but not exclusively (Begon 2018), not trivial (SARA, SCoRE \cite{Ehrig2007} if STA via functional methods, midpoint between anatomical markers via geometric). The mere concept of joint centers is also not always pertinent, as human and animal joints are complex and translate blabla. 
Potentially with inertial properties, as well as neural and/or muscular system.

INITIALISATION
3. Bonne initialisation (angles, voire vel=0?, accel=0? pas généralement vrai, covariances with Kalman which is more sensitive to bad initial estimates)
...
\begin{table}[!htbp]
  ...
  \multicolumn{3}{c}{\emph{By OpenSim \cite{Delp2007}, \cite{Visual3D}, KineticsToolkit \cite{Chenier2021}}} \\
  \midrule
  \specialrule{0.14 em}{0pc}{0pc}
  \multicolumn{3}{c}{\textbf{Multi-body Inverse Kinematics}} \\
  \midrule
  \textbf{Levenberg-Marquardt (Jacobian)}   & \textbf{L-BFGS-B (Quasi-Newton)}   & \textbf{Extended Kalman Filter}    \\
  \midrule
  Fast   &  Robust &    \\
  Unconstrained   &  Constraints ok & Constraints ok   \\
  \emph{Visual3D, \cite{Pinocchio}, Custom \cite{Muller2019}}  &  \emph{OpenSim, Visual3D, Pyomeca \cite{Martinez2020} with OpenSim API, (Custom)}  &  \emph{OpenSim Fork: KS, Pyomeca with Biorbd library: EKF} \\
  \bottomrule
  \end{tabular}
