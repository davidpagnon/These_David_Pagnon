...
\vspace*{0.5cm}
\noindent\textbf{Calibration from one single image}
...
\noindent\textbf{Calibration from several images: Zhang method.} 
% The previous section has shown how to estimate the camera pose from a single image. However, in practice, the camera pose is estimated from several images, and the camera pose is then refined by minimizing the reprojection error. This is done by minimizing the sum of the squared reprojection errors, which is a non-linear optimization problem. The solution is found by an iterative algorithm, such as the Levenberg-Marquardt algorithm \cite{Levenberg1944, Marquardt1963}. This algorithm is implemented in OpenCV with the \href{https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html#ga549c2075fac14829ff4a58bc931c033d}{solvePnP()} function. The algorithm is initialized with the camera pose estimated from the first image, and then refined with the camera pose estimated from the second image, and so on. The algorithm stops when the reprojection error is below a threshold, or when the maximum number of iterations is reached. The reprojection error is the Euclidean distance between the 3D point and the 2D point projected on the image plane. The reprojection error is computed with the \href{https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html#ga6b7a3d5f3f0f0c3e9e0d0f0d3d7d5c1b}{projectPoints()} function. The reprojection error is computed for all the 3D points, and the sum of the squared reprojection errors is minimized. The reprojection error is also used to compute the confidence of the camera pose, which is the probability that the camera pose is correct. This is done by computing the probability density function (PDF) of the reprojection error, and then computing the probability that the reprojection error is below a threshold. This is done with the \href{https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html#ga0f0f0f9a5f43989e4d4dcbca185a54d8}{computeReprojectionErrors()} function. The confidence is then computed with the \href{https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html#ga0f0f0f9

In practice, a more robust method would use more than one single image of a checkerboard for calibration. A classic approach is the one proposed by \cite{Zhang2000}. It can be separated into 3 steps: first, 

, which is assumed to be place at the origin of the world  has the advantage of having regularly spaced corners of known dimensions, and of being planar. Z = 0
...
shortly before, or after, or during the capture of the image
...
If distortions are significant, undistort before triangulation (alternatively, GoPro linear mode on the fly)
