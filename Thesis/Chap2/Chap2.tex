...
One particular case of deep neural networks is the convolutional neural network (CNN), which is particularly suited for image recognition. 

% AI boom 21st century? 

% It is based on the idea that the visual cortex of the brain is organized in layers, with the first layers detecting simple features such as edges or corners, and the last layers detecting more complex features such as real-life objects. The first layers are therefore called convolutional layers, and the last layers are called fully connected layers. The CNN is trained with a large number of images, and the weights of the network are adjusted so that the network can correctly classify new images. 

% cnn for computer vision
% Graph
% Small window, sliding across the image, acting as a filter detecting simple features such as corners or edges. The output of the filter is a feature map, which is then fed to the next layer. The weights of the filter are adjusted so that the network can correctly classify new images. The last layer is a fully connected layer, which is a standard neural network layer. The output of the network is the probability that the image belongs to a given class.
% using gradient to classify image areas as corners and edges (overlaying, matching template of simple features such as a corner or an edge?)
% Late layers: superimpose complex features corresponding to whole objects

% Transfer learning (hundreds of images instead of thousands)
...
2010  data-driven methods definitely took over knowledge-driven ones in vision analysis problems, and by extension in sports analysis from videos.
