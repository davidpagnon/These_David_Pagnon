...
More specifically, in the field of inverse kinematics, the L-BFGS-B variant is often used. This is a limited-memory of the BFGS method, which uses a sparse approximation of the Hessian instead of a dense one, further adapted for handling joint limits by gradient projection \cite{Byrd1995,Zhao1994}. Since the Jacobian is not inverted, these methods return the most accurate results, and stay free from any singularity issues like with the Levenberg-Marquardt method. However, they are still more computationally costly than Jacobian methods, and are not as straightforward to implement \cite{Aristidou2018}. They are used in the open-source software applications OpenSim, and in Visual3D. Pyomeca \cite{Martinez2020} relies on the OpenSim Python API to provide such a method, via its library Pyosim. CusToM uses a Sequential Quadratic Programming method, which is related to Quasi-Newton ones.


\newpage
\subsubsection{Kalman filtering methods}

Another lesser-known class of inverse kinematic methods is the Kalman filter, which does not take its roots from the fields of robotics, nor computer animation, nor biomechanics, but from automation. In fact, it was precisely designed to mitigate the effect of noisy measurement data on the estimation of variables of interest \cite{Kalman1960}. Kalman filters take advantage of prior knowledge of both the evolution of kinematic variables along time (namely, the process model), and of the kinematic chain (namely, the measurement model). The process model concerns the change of joint parameters from one frame to another, as well as the magnitude of imprecision on this predicted evolution; and the measurement model concerns the relationship between marker positions and joint angle parameters, as well as the magnitude of errors (due to STA for example). 

In short, an inverse kinematics Kalman filter will proceed in two steps. It will first predict the angular joint parameters of the next frame thanks to the process model. Then, it will update this prediction, by deducing the corresponding marker positions thanks to the process model, and comparing these to the actual measured marker coordinates. This difference will be weighted by the Kalman gain, computed at each frame, and added to the initial joint parameter estimate. A low gain means that the initial estimate can be trusted, and a higher gain places more weight on marker measurements.

Let $\overrightarrow{X} = (x_1, y_1, z_1, x_2, \cdots, z_M)^T_{1 \times 3M}$ be the measured marker coordinates, called the output vector, and $\overrightarrow{q} = (\theta_1, \dot{\theta_1}, \ddot{\theta_1}, \theta_2, \cdots, \ddot{\theta_N})^T_{1 \times 3N}$ the joint angle coordinates, velocities, and accelerations, called the state vector. Let $\overrightarrow{\eta(t)}$ be the process noise, assumed to be a zero-mean Gaussian distribution with covariance $\textbf{Q}_t$. The \emph{process model} is expressed as: 
\begin{flalign}
...
Let $\overrightarrow{\nu(t)}$ be the zero-mean measurement noise of covariance $\textbf{R}_t$, the \emph{measurement model} is expressed as: 
\begin{flalign}
...
In the \emph{prediction phase}, considering that the mean process noise is null, an estimate for $\overrightarrow{q(t+\Delta t)}$ is:
\begin{equation}
...
\end{equation}
The covariance of prediction $\hat{\textbf{P}}_{t+\Delta t}$ is also calculated at this stage. As it is lengthy to expose and slightly more complicated, we won't detail it here (see \cite{Bonnet2017} for further details). 

The \emph{update phase} starts with computing the difference between measured marker positions $\overrightarrow{X^{target}}$, and those deducted from joint angular parameter estimates :
\begin{equation}
    \Delta \overrightarrow{X(t+\Delta t)} 
    = \overrightarrow{X^{target}(t+dt)}
    - h\bigl(\overrightarrow{\hat{q}(t+\Delta t)}\bigr)
\end{equation}
The covariance of error $\textbf{S}_{t+\Delta t}$ is more complicated to compute: as $h$ is non-linear, it has to be approximated with a first order Taylor expansion. Both prediction and error covariances $\textbf{Q}_{t+\Delta t}$ and $\textbf{R}_{t+\Delta t}$ are involved in the computation of the Kalman gain $\textbf{K}_{t+\Delta t}$.

Finally, the update joint angular parameter estimates are:
\begin{equation}
\boxed{
...
Good alternative approach when some segments are hidden and when closed loops (kin constraints easier) Halvorsen2008
--> extended Kalman filter with kinematic constraints (EKF)

Pourquoi pas utilisé ?
Difficile à comprendre, à implémenter ?  à paramétriser ?
Besoin d'initialiser correctement (surtout le root segment)
Problèmes si erreurs dans modèle (joint centers pas bien au center ?)
Very slow? Calculs sur tous les markers à chaque frame, chq calcul combinaison complexe sin cos, + process model
+ hard to initialize (coord, vel, acc)
-----

OpenSim fork, Pyomeca \cite{Martinez2020} biorbd \cite{Michaud2021}
...
% Simbody: https://raw.githubusercontent.com/simbody/simbody/master/Simbody/doc/SimbodyTheoryManual.pdf
...
inverse-> trust model
Although we skimmed it, scaling the model is indispensable for IK to reach des résultats corrects. Not trivial: besoin de trouver les joint centers, plusieurs manières
...
- heuristic (CCD: abrupt movements, twisted last joint, works bad if more than a few joints; Fabrik: good first iteration, others slower)
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{ccc}
  \toprule
  \multicolumn{3}{c}{\textbf{Single-body kinematic optimization}} \\
  \midrule
  \multicolumn{3}{c}{Trust markers} \\
  \multicolumn{3}{c}{\emph{By OpenSim, Visual3D, KineticsToolkit}} \\
  \midrule
  \specialrule{0.14 em}{0pc}{0pc}
  \textbf{Levenberg-Marquardt (Jacobian)}   & \textbf{L-BFGS-B (Quasi-Newton)}   & \textbf{Extended Kalman Smoother}    \\
  \midrule
  Fast   &  Robust &    \\
  Unconstrained   &  Constraints ok & Constraints ok   \\
  \emph{Visual3D, Pinocchio, Custom}  &  \emph{OpenSim, Visual3D, Pyomeca, (Custom)}  &  \emph{OpenSim (fork), Pyomeca} \\
  \bottomrule
  \end{tabular}
  \caption{Comparison of kinematic approaches.}
\end{table}
