...
\begin{algorithm}[!ht]
      ...
      \begin{algorithmic}[1]
        ...
        \end{equation}
        \STATE This result is processed by an activation function, which is a threshold in the case of the perceptron. It determines whether the neuron will be fired or not, i.e., whether one or the other class will be predicted. $\sigma^{0,pred}$ = 1 corresponds to one class, and $\sigma^{0,pred}$ = 0 to the other. 
        \begin{equation}
            \sigma^{0,pred} = 
            \begin{cases}
...
\begin{algorithm}
      \begin{algorithmic}[1]
        \algrestore{alg1}
        \STATE This prediction $\sigma^{0,pred}$ is compared to the actual class $\sigma^{0,expected}$. 
        \begin{equation}
            \epsilon^0 = \sigma^{0,expected} - \sigma^{0,pred}
        \end{equation}
      ...
      %   \STATEx \textit{In Adaline, weights are adjusted early and in a continuous way, before being processed by the activation function: \(\epsilon^0 = \frac{\partial \frac{1}{2}(\sigma^{0,expected} - \overrightarrow{W^0} \cdot \overrightarrow{X^0})^2}{\partial \overrightarrow{W^0}}= \sigma^{0,expected} - \overrightarrow{W^0} \cdot \overrightarrow{X^0}\)}.
...
\begin{tcolorbox}[colback=white,colframe=black, colbacktitle=white, coltitle=black, breakable, title=\textbf{Example 1} Athlete classification with a perceptron, label=example1]
      ...
      \[ \bigl\{(\overrightarrow{X^i}, \sigma^{i, expected})\bigr\}_{i\in [0,4]}
      ...
      \(\sigma^{0, expected} = 1 = \sigma^{0, pred} \), so the prediction is true and weights don't need to be updated. \newline
      ...
      But \(\sigma^{1, expected} = 0 \neq \sigma^{1, pred} = 1\), so weights need to be updated.\newline
      The error is \(\epsilon^1 = \sigma^{1,expected} - \sigma^{1,pred} = 0-1 = -1\).\newline
      ...
      \(\epsilon^2 = \sigma^{2,expected} - \sigma^{2,pred} = 1\).\newline
      ...
      \(\epsilon^3 = \sigma^{3,expected} - \sigma^{3,pred} = -1\).\newline
      ...
      \(\sigma^{4, expected} = 1 = \sigma^{4, pred} = 1\), so weights don't need to be updated.\newline
...
From the end of the 1990s, there has been no theoretical breakthrough in AI, but larger databases have become available with the advent of the world wide web, and greater computational power has become accessible, especially thanks to groundbreaking progress in Graphics Processing Units (GPUs), which made heavy parallel computing available to the wider audience. As a consequence, more layers could be used in neural networks, which set off the advent of deep learning. 


% cnn for computer vision
% Graph

% This has led to a new rise of AI, with the development of deep learning methods. These methods are based on the same principles as the perceptron, but are much more powerful. They are able to learn from large amounts of data, and to solve complex problems. They are also able to learn from unstructured data, such as images, videos, and text. This has led to a new wave of applications in many fields, such as self-driving cars, speech recognition, and medical diagnosis. However, the development of deep learning methods is still in its infancy, and many challenges remain to be solved. For example, deep learning methods are still not able to learn from unstructured data such as music, and to generate new music. This is a very complex problem, and it is not clear if it will ever be solved. Nevertheless, the development of deep learning methods has led to a new AI winter, as many companies have overestimated the potential of these methods, and have invested in them without a clear understanding of their limitations. This has led to a new wave of disappointment, and to a new wave of cuts in fundings. This is why we are currently in the third AI winter.

% and the AI winter of the 1980s followed. The field was saved by the development of the Internet, which allowed for the collection of massive amounts of data, and the development of GPUs, which allowed for the training of deep neural networks. The second AI winter was followed by the third one, in 2015, when the hype around deep learning was followed by a series of failures in the industry. The field is now in a new rise, with the development of new architectures, such as Transformers \cite{Vaswani2017}, and the availability of large datasets, such as ImageNet \cite{Deng2009}.




% vanishing gradient, overfitting (data augmentation, regularization, dropout, batch normalization), 
% CNN
% Deep learning
