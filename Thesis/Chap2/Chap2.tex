...
Now, classification of a whole image is not sufficient in sports motion analysis. One needs to detect where an object or a person is (this is called detection), ideally to outline the result pixel by pixel (this is segmentation, and is out of the scope of this thesis), to localize more precise features such as joint centers so as to estimate the person's pose (this is keypoint detection). Older detection algorithms 
(Figure~\ref{fig_linearly_sep})



% This is called object detection. The first CNNs for object detection were based on the sliding window method \cite{Dalal2005}, which consists in sliding a small window across the image, and classifying each window as containing an object or not. This is very computationally expensive, and the results are not very good. 
...
% Image: Classification whole image, Detection objects Bounding box, segmentation outline, keypoint detection smaller parts.
