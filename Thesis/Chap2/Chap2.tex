...
If only 3 markers are used per segment, the problem can be solved analytically. The method is then called Direct Kinematics (see \hyperlink{Ann:gloss}{Disambiguation}) \cite{Lu1999}. The local reference frame of each segment can be calculated, and all inter-segmental rotations and translations can be found, using trigonometric formulas. 
...
This problem is highly non-linear since $\overrightarrow{X^{current}_m(\overrightarrow{q})}$ is calculated as a complex sine and cosine combination of $\overrightarrow{q}$, and becomes increasingly hard to compute as the number of degrees of freedom increases. Consequently, iterative methods have to be employed. 3 main approaches coexist: Jacobian methods such as the Levenberg-Marquardt algorithm, Quasi-Newton methods such as the BFGS one, and Kalman filtering methods.
...
In the fields of robotics and character animation, inverse kinematics is classically solved by Jacobian methods \cite{Siciliano1990,Buss2009,Aristidou2018}. They consist in linearizing marker coordinates around an initial guess on joint angles, and solving the resulting linear system. %The guess is then iteratively adjusted, until the marker coordinate error is small enough. 

Let $\overrightarrow{q^{init}}=(\theta_1^{init}, \dots \  \theta_N^{init})$ be the N initial joint coordinates, and \\
$\overrightarrow{X(\overrightarrow{q^{init}})} = \bigl(x_1(\overrightarrow{q^{init}}), y_1(\overrightarrow{q^{init}}), z_1(\overrightarrow{q^{init}}), \dots \ z_M(\overrightarrow{q^{init}})\bigr)$ the 3M marker initial coordinates. A first order Taylor expansion of $\overrightarrow{X}$ around $\overrightarrow{q^{init}}$ yields:
\begin{equation}
...
\end{equation} 
The goal is to find an $\overrightarrow{X^{current}(\overrightarrow{q})} = \overrightarrow{X(\overrightarrow{q^{init}}+\Delta\overrightarrow{q})}$ which minimizes Equation~\ref{eq_obj_ik}. By slightly abusing the "squared" symbol, the objective function will be expressed as:
\begin{equation}
  \textbf{W} \ 
  \Bigl(\overrightarrow{X^{target}} - \overrightarrow{X(\overrightarrow{q^{init}})} - \textbf{J} \ \Delta\overrightarrow{q} \Bigr)^2
\end{equation}
It is minimum when its gradient (with respect to $\Delta\overrightarrow{q}$) is annulled, i.e., when:
\begin{equation}
  -2 \textbf{J}^T \ \textbf{W} \ 
  \Bigl(\overrightarrow{X^{target}} - \overrightarrow{X(\overrightarrow{q^{init}})} - \textbf{J} \ \Delta\overrightarrow{q} \Bigr)
  \approx 0
\end{equation}
Or more simply, with $\Delta\overrightarrow{X} = \overrightarrow{X^{target}} - \overrightarrow{X(\overrightarrow{q^{init}})}$, when:
\begin{equation}
  \boxed{
...
\begin{enumerate}[itemsep=0em, topsep=0em, leftmargin=*]
    ...
    \end{equation} 
    This is analogous to the \emph{gradient descent method}. $\textbf{J}^T$ is quick and easy to compute, but the algorithm converges slowly, and pose results are sometimes unsatisfying.
    ...
    \end{equation} 
    This is analogous to the \emph{Gauss-Newton method}. In fact, this result can be found by replacing the linearized coordinates of $\overrightarrow{X^{current}(\overrightarrow{q^{init}})}$ (Equation~\ref{eq:gausslin}) into the objective function (Equation~\ref{eq_obj_ik}), and then solving it by determining where its gradient is null, like in a typical linear least-square problem. 
    
    % The confidence given to each marker can be taken into account by multiplying marker coordinates by their diagonal matrix of weights \textbf{W} \cite{Meredith2005}:



    This method converges faster, but is sensitive to bad initial estimates and unstable near singularities, i.e., $\Delta \overrightarrow{q}$ becomes arbitrarily large when $\textbf{J}^+$ is close to zero. Moreover, the pseudo-inverse can be expensive to compute. One way to mitigate this drawback consists in calculating it with an SVD. The Jacobian can be decomposed as $\textbf{J} = \textbf{U} \ \textbf{S} \ \textbf{V}^T$ (see Algorithm~\ref{alg:svd}). Then:
    \begin{equation}
          \begin{aligned}
          \textbf{J}^+
...
In plain words, the gradient descent method iteratively searches the direction of steepest gradient, until the objective function reaches a minimum where the gradient is null; the Gauss-Newton method linearizes the problem, takes advantage of the sum-of-squares formulation of the objective function, and directly finds where the gradient is close to zero; and the Levenberg-Marquardt method is a compromise between the two.

It uses the result of the previous frame as an initial guess, which is arguably close to the desired solution. Hence, it is generally fast and can be used in real-time in 3D animation software applications. However, it tends to be sensitive to singularities, and to be less precise than Quasi-Newton methods. Moreover, joint limits are not straightforward to take into account \cite{Aristidou2018}. The Levenberg-Marquardt method is implemented in the open-source softwares Custom and \cite{Pinocchio,Carpentier2019}, as well as in Visual3D. 

\newpage
\subsubsection{Quasi-Newton methods and the BFGS algorithm}

In the field of biomechanics, inverse kinematics is usually dealt with Quasi-Newton methods. These methods don't linearize the objective function, and solve it as a more general non-linear minimization problem, which is not restricted to sum-of-squares objective functions. In short, instead of minimizing $\overrightarrow{X^{target}_m} - \overrightarrow{X^{current}_m(\overrightarrow{q})}$, Newton methods search for the direction where it decreases the fastest. In other words, they search for the direction where its gradient $\nabla ( \overrightarrow{X^{target}_m} - \overrightarrow{X^{current}_m(\overrightarrow{q})})$ is the highest. This can be found by taking a second order Taylor expansion of $\overrightarrow{X}$ around $\overrightarrow{q^{init}}$:


% The gradient is computed using the Jacobian matrix, and the Hessian matrix is approximated using the BFGS algorithm. The BFGS algorithm is a quasi-Newton method that approximates the Hessian matrix using the gradient of the objective function. It is implemented in the open-source softwares OpenSim and Custom, and in Visual3D.

maximize 

search the direction in which it decreases the fastest, i.e., where its gradient is the highest.

Approximation de la dérivée seconde (dropped avec les approches précédentes)

Second order, less iterations but each of them much more computationally expensive. More general, no need for sum of squares
BFGS version de SQP (Sequential Quadratic Programming) plus rapide, qui approxime le hessien
constraints ok
