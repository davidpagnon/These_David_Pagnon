...
\vspace*{0.5cm}
\noindent\textbf{Wand calibration with Sparse Bundle Adjustment (SBA)} 

When using several cameras, they need to share a common frame of reference. Under those circumstances, the checkerboard needs to be visible by all cameras on one frame at least 

extrinsic parameters need to be seen

The Zhang method performs better when the checkerboard is visible from  images are seen by 
When using several cameras, one image of the checkerboard needs to be seen clearly by all the cameras together


the extrinsic parameters need to be shared by 
The frame used for extrinsic
When cameras are disposed around athletes, extrinsic calibration with a checkerboard can be complicated. The checkerboard needs to be seen by all cameras simultaneously, 

Wand peut plus facilement être visible de toutes les caméras, alors qu'avec un checkerboard on court généralement le risque de montrer son dos. Problème: pas de coordonnées 3D comme avec 2D plan, parce que 1D object: sparse variant of Levenberg-Macquardt algo, robust to missing points, done over numerous points  

Wand: TABLE ? voir articles + protocole.docx
- Résultats similaires à checkerboard, sub-pixel
- Much less cumbersome (transportation). 
- Plus simple: Utilisée par quasi toutes les solutions commerciales + easywand + argus.
- Position relative des cams calculée sur toutes les frames, puis global coordinate system avec autre objet. Pas besoin que toutes les cams voient l'origine. Checkerboard: position de chaque cam par rapport à checkerboard sur une seule frame, pas de position relative: si une caméra ne détecte pas l'origine (trop petit, ou pas bien vu quand à plat), pas utilisable. Ou alors, utiliser plus grand object et solvePnP cf chap 6
- Calibre toutes les caméras ensemble: 1. calculs intenses, heureusement, Jacobien creux. 2. Besoin d'initialisation solide (plus général, plus de variables à optimiser), avec L frame par exemple pour calib initiale, ou matrice intrinsèque précalculée. 3. besoin de synchronisation
- Moins bien pour distorsion
- 2 markers durs à différencier de l'arrière plan, contrairement à motif complexe comme checkerboard (mais checkerboard pour extrinsèque devrait être grand)
- No good library in Python




SBA sparse bundle adjustment \cite{Lourakis2009}: EasyWand \cite{Theriault2014} or Argus \cite{Argus,Jackson2016} (portabilité pour python)

Matlab toolbox Bouguet J.-Y. (1999). Visual Methods for Three-Dimensional Modeling. PhD thesis, California Institute of Technology, Pasadena, CA, USA -> sur quoi se base opencv ?

OpenCV = portabilité python de Bouguet matlab 
...
Wand se limite à 2 points, qui forment une seule ligne 1D et ne suffisent pas à calculer ces paramètres. Il faut donc faire une triangulation a priori, et minimiser peu à peu l'erreur de reprojection au fur et à mesure qu'on lui donne les deux points suivants image par image (par optimisation par sparse bundle adjustment, SBA). Une fois que l'erreur est assez faible, on peut calculer les paramètres extrinsèques par SVD. C'est donc plus lent, et généralement moins exact.


\vspace*{0.5cm}
\noindent\textbf{In practice}  

Intrinsic and distortion at any moment, in theory constant for a given camera. With several images of checkerboard, close to the camera, various orientations.

Extrinsic depends on position: done at every new capture, considering that once cameras have been moved, it is usually too late (even though some options such as presented chapter 6 are possible, as well as autocalibration, which is not standardized enough to be detailed here. See cite for more details). With one image, must be seen by all cameras, and large enough to be correctly detected. 

Zhang computes them simultaneously for all image view, but if intrinsic camera parameters already determined, possible to calibrated extrinsic with one single image of any 3D object of known dimensions: solvePnP \cite{Marchand2015} 
...
- ADD DLTDV8 p10 avec Kinovea, et dans tableau. can also triangulate if cams calibrated, on matlab (not free)
...
Assumes cameras not only calibrated, but also synchronized. Classically hardware, can be on flash, clap (even if speed of sound is finite), wireless signal such as WiFi, bluetooth or GPS, or other approaches based on cross-correlation of feature speeds on each cameras, such as proposed chapter 6
