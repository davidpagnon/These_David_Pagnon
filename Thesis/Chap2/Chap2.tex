...
% It is based on the idea that the visual cortex of the brain is organized in layers, with the first layers detecting simple features such as edges, and the last layers detecting more complex features such as faces. The first layers are therefore called convolutional layers, and the last layers are called fully connected layers. The CNN is trained with a large number of images, and the weights of the network are adjusted so that the network can correctly classify new images. The CNN is a very powerful tool, but it is not the only one. In the following sections, we will see that other deep learning methods can be used for image recognition, and that they can be combined to achieve even better results.
