...
Some corrections can be brought to this model. First, then optical center may not be at the exact center of the image plane. In this case, the relation becomes:
\begin{equation}
  \begin{aligned}
...
% First, the camera is not perfectly aligned with the optical axis, and the image is not perfectly centered. This can be corrected by adding a translation vector \(t\), which is the distance between the camera center and the image center. Second, the camera is not perfectly parallel to the image plane, and the image is not perfectly perpendicular to the image plane. This can be corrected by adding a rotation matrix \(R\), which is the rotation between the camera and the image plane. The final model is thus:
...
% The camera frame of reference is defined by the camera position \(P_C\), and the camera orientation \(R_C\), which is the rotation matrix that transforms a vector from the camera frame of reference to the inertial frame of reference. The camera position and orientation are usually obtained from a calibration process, which is out of the scope of this book. The camera frame of reference is usually expressed in the form of a homogeneous transformation matrix \(T_C\), which is the concatenation of the rotation matrix \(R_C\) and the translation vector \(P_C\). The homogeneous transformation matrix \(T_C\) is defined as:
